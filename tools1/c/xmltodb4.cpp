...
char strinsertsql[10241]; //插入表的sql
char strupdatesql[10214]; //更新表的sql
CLogFile logfile;
...
void crtsql()
{
	...
	{
		if((strcmp(TABCOLS.m_vallcols[i].colname,"upttime")==0) ||(strcmp(TABCOLS.m_vallcols[i].colname,"keyid")==0)) continue;
	...
	SNPRINTF(strinsertsql,10240,sizeof(strinsertp1),\
					"insert into %s(%s) value(%s)",stxmltotable.tname,strinsertp1,strinsertp2);
	...
	//然后再拼接where后面的部分
	strcat(strupdatesql,"where 1=1");  //用1=1是为了格式的方便，只要and加语句即可
