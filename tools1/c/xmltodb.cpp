...
int totalcount,inscount,uptcount; //总记录数等
...
bool _xmltodb()
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      if (iret==0)
      {
        logfile.WriteEx("ok(%s totalcount=%d,insert=%d,uptcount=%d).\n",stxmltotable.tname,totalcount,inscount,uptcount);
      ...
      if ( (iret==1) || (iret==2)||(iret==5) )
      {
        ...
        if (iret==2) logfile.WriteEx("failed，待入库的表（%s）不存在。\n",stxmltotable.tname);
				if (iret==5) logfile.WriteEx("failed，字段数超过最大值。\n");
...
if(strcolvalue[i]!=0) { delete strcolvalue[i];strcolvalue[i]=0;}
  ...
  if (TABCOLS.m_allcount==0) return 2; // 待入库的表不存在。
	...
	for(int i=0;i<TABCOLS.m_allcount;i++) strcolvalue[i]=new char[TABCOLS.m_vallcols[i].collen+1];
