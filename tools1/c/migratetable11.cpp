...
bool _migratetable()
{
  ...
  char keyvalues[starg.maxcount][51];   // 存放唯一键字段的值。
	...
	// 准备删除和插入数据的SQL，一次删除MAXPARAMS条记录。
  // 插入的sql，insert into table(a,b,c) select a,b,c,from tname where in (:1,:2);
  // 删除的sql, delete from table where key in(:1,:2);
  // 注意，mysql需要2个prepare语句，否则会报错	
