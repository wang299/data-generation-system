...
vector<int> sockqueue;														// 客户端队列的容器 

// 线程信息的结构体
struct st_pthinfo
{
	pthread_t pthid;// 线程id
	time_t atime;   // 最近活动的时间
};

pthread_t checkpthid;			// 监控线程的id
pthread_t checkpoolid;			// 监控数据库连接池的id
pthread_spinlock_t spin;  // 用于锁定vthid的自旋锁。
vector<st_pthinfo> vthid;      // 存放全部线程信息的容器。
...
int main(int argc,char *argv[])
{
  ...
  if (TcpServer.InitServer(starg.port)==false)
  {
	...
	// 创建监控线程
 	if (pthread_create(&checkpthid,NULL,checkthmain,(void*)(long)0)!=0)
   {
     logfile.Write("pthread_create() failed.\n");  return -1;
   }
	...
	while (true)
  {
    ...
    logfile.Write("客户端（%s）已连接。\n",TcpServer.GetIP());
		...
		// 把客户端的socket放入队列，并发发送条件信号。
		pthread_mutex_lock(&mutex);								// 加锁
		sockqueue.push_back(TcpServer.m_connfd);	// 将客户端的socket入队
		pthread_mutex_unlock(&mutex);							// 解锁
		pthread_cond_signal(&cond);								// 发送信号，激活线程
  ...
  pthread_detach(pthread_self());           // 把线程分离出去。
	...
	while(true)
	{
		...
		while(sockqueue.size()==0)
		{
			...
			now.tv_sec=now.tv_sec+20;
			pthread_cond_timedwait(&cond,&mutex,(struct timespec*)&now);// 等待条件触发
		...
		// 业务处理代码
		logfile.Write("phid=%ld(num=%d),connfd=%d.\n",pthread_self(),pthnum,connfd);
	
		// 读取，失败或者超时，关闭客户端sock，回到循环
   ...
   if(CheckPerm(conn,strrecvbuf,connfd)==false) {oraconnpool.free(conn); close(connfd);continue;}
    ...
    Writen(connfd,strsendbuf,strlen(strsendbuf));
  	...
  	if(ExecSQL(conn,strrecvbuf,connfd)==false) 
		{oraconnpool.free(conn);  close(connfd);continue;}	
...
void EXIT(int sig)  
{
  ...
  sleep(1);        // 让子线程有足够的时间退出。
	...
	pthread_cancel(checkpoolid);// 取消检查数据库连接的线程
...
void *checkthmain(void *arg)
{
	while(true)
	{
		...
		{
			if(time(0)-vthid[i].atime>25)
			{
				// 已经超时
				logfile.Write("thread %d(%lu) timeout(%d)\n",i,vthid[i].pthid,time(0)-vthid[i].atime);
