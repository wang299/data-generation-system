...
connection conn;     // 用于执行查询SQL语句的数据库连接。

// 业务处理主函数。
bool _migratetable_oracle();
...
int main(int argc,char *argv[])
{
  ...
  if (conn.connecttodb(starg.connstr,NULL) != 0)
  {
	...
	logfile.Write("connect success\n");

  // 业务处理主函数。
  _migratetable_oracle();
}
...
void _help(char *argv[])
{
  ...
  printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/migratetable_oracle /log/idc/migratetable_oracle_ZHOBTMIND.log \"<connstr>qxidc/qxidcpwd@oracle_wyz</connstr><srctname>T_ZHOBTMIND</srctname><dsttname>T_ZHOBTMIND_HIS</dsttname><keycol>keyid</keycol><where>where ddatetime<sysdate-0.03</where><starttime>01,02,03,04,05,13</starttime><maxcount>300</maxcount><timeout>120</timeout><pname>migratetable_oracle_ZHOBTMIND</pname>\"\n\n");
...
bool _migratetable_oracle()
{
  ...
  if (TABCOLS.allcols(&conn,starg.dsttname)==false)
  {
	...
	logfile.Write("表%s存在。%s\n",starg.dsttname,TABCOLS.m_allcols);
